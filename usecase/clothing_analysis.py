...
def download_image_from_line(line_bot_api, message_id, temp_dir):
    """LINEから画像をダウンロードして一時ファイルとして保存する"""
    message_content = line_bot_api.get_message_content(message_id)
    
    # 一時ファイル名を生成（UUIDを使用して一意のファイル名を生成）
    temp_file_path = temp_dir / f"{uuid.uuid4()}.jpg"
...
def generate_clothing_description(features):
    """抽出した特徴から服の説明文を生成する"""
